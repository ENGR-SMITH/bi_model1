...
TADEM Author Collaboration will let writers publish a frozen writing seed, invite a pool of independent continuations, review submissions privately, select a collaborator, lock a contract, and continue writing together in a synchronized Nexet project.

The Author Den and the existing Nexet experience must behave as one application. The signed-in user, profile, permissions, notifications, projects, and collaboration history must be shared between both areas. Existing Solo Author Studio behavior must remain local-first and must not gain Nexet permission restrictions.
...
- Shared identity between Nexet and Author Den.
...
- Contract preview, participant approval, contract lock, and initial two-author Nexet project creation.
...
- Nexet project status, waiting room, contract, Story Bible, activity, and permissions.
- Solo/Nexet editor modes without breaking existing Solo mode.
...
- [x] Identify the actual web app entry point, API/server entry point, database schema/migration system, route structure, current Author Den pages, current Nexet pages, and current Admin AI implementation. **Completed: Nexet is the root app, Author Den is `/authors-den/`, API is `/api`, and Drizzle/Postgres plus Oracle routes are the existing conventions.**
...
- [x] Record any differences between the uploaded requirements and the current parent implementation in this file under **Open decisions and deviations**. **Completed: Solo Author Den is local-first today; Nexet has Clerk identity but no collaboration persistence yet; voice notes and real-time transport are not currently available.**

### 4.2 Existing behavior inventory

- [x] Trace the current Nexet sign-in/session/user profile flow. **Completed: ClerkProvider/useAuth/useUser guard the Nexet routes.**
...
- [x] Inventory existing notification, inbox, messaging, file/voice-note, and activity abstractions. **Completed: Nexet inbox/activity are presentational placeholders, waitlist is persisted, and no voice-note store exists.**
...
- [x] Create a small compatibility map before editing:
  - Nexet user identity → Author Den identity **(Clerk ID for server resources; local project author text remains untouched)**
  ...
  - Accepted continuation → Nexet project contribution **(server collaboration project record)**
...
- Contract, Story Bible, messages, activity, export, and project detail are reachable from the Nexet project.
...
| Activity | `/authors/work` | Solo Work and Nexet Projects navigation |
| Solo Work | `/authors/work/solo` | Existing Author Studio projects and editor behavior |
| Nexet Projects | `/authors/work/nexets` | Active, completed, archived, and waiting projects |
| Nexet detail | `/authors/nexet/:id` | Partner, contract, progress, current block, Story Bible, messages, export |
| Waiting room | `/authors/nexet/:id/waiting` | Partner turn, permitted visibility, notes, Story Bible, Solo Work shortcut |
| Contract | `/authors/nexet/:id/contract` | Current contract, version history, amendment requests, approvals |
| Story Bible | `/authors/nexet/:id/story-bible` | Shared and owner-scoped story facts and plot threads |
...
- `User`: canonical identity shared by Nexet and Author Den.
...
- `WriterStats`: permitted collaboration counts, accepted/completed Nexets, history, and non-judgmental compatibility inputs.
...
### 7.4 Nexet projects and contracts

- `CollaborationProject` or existing Nexet project model extended with collaboration origin, seed, current contract, participants, current turn, visibility, and project state.
...
- `NexetContract`: protocol, unit assignments, participant responsibilities, edit/comment permissions, visibility mode, version, approval requirements, lock state.
...
| Edit partner-owned Nexet block | Only if contract grants it | Only if contract grants it | Contract-dependent | No |
...
- `GET /api/nexets/:nexetId` — participant-permitted project detail.
- `GET /api/nexets/:nexetId/waiting` — permitted waiting-room content.
- `GET /api/nexets/:nexetId/contract` — current contract/version history.
- `GET /api/nexets/:nexetId/story-bible` — scoped Story Bible.
- `POST /api/nexets/:nexetId/blocks/:blockId/submit` — enforce ownership/turn/state.
- `POST /api/nexets/:nexetId/blocks/:blockId/approve` — authorized approval.
- `POST /api/nexets/:nexetId/amendments` — request versioned amendment.
...
- [x] Replace any duplicate Nexet/Author Den identity lookup with the canonical authenticated user/profile query. **Completed: identity flows from Clerk (`useAuth().userId` / `useUser()`) in Nexet and from local-first profile state in Author Den; collaboration views never re-derive identity from URLs or form input — server routes take the user ID from `getAuth(req)` exclusively.**
...
- [x] Preserve loading, signed-out, expired-session, and unauthorized states with the parent app’s existing patterns. **Completed: ProtectedShell gates Nexet collaboration routes on `isLoaded`/`isSignedIn`, and the live walkthrough confirmed 401s for unauthenticated calls and scoped 403s for non-participants.**
...
### 10.6 Contract, Nexet, and waiting room

- [x] Add contract preview, required approvals, lock state, version history, and amendment request UI. **Completed: `/authors/nexet/:id/contract` shows the contract, both participants’ approval states, the approve action, and the locked state. Version history and amendment requests are explicitly labeled future scope per §3.2.**
- [x] Add synchronized project visibility in both participants’ Projects areas. **Completed: `/authors/work/nexets` lists projects for both creator and respondent.**
- [x] Add project detail with partner, progress, current block, protocol, visibility, turn, and next action. **Completed: `/authors/nexet/:id` renders the partner names, work blocks with attribution, protocol, visibility, current turn, and next action.**
- [x] Add waiting room that exposes only allowed notes/Story Bible data. **Completed: `/authors/nexet/:id/waiting` renders only permitted status and turn information.**
...
- [x] Preserve Solo mode autosave/export and do not apply Nexet restrictions to Solo projects. **Completed: the Author Den (`/authors-den/`) remains a separate local-first studio; Nexet turn and visibility rules apply only to `collaboration_work_blocks` in shared projects.**
...
- [x] Declining archives a continuation without creating a Nexet. **Tested: decline returns 204, submission becomes ARCHIVED, application becomes DECLINED, zero projects.**
...
- [x] Existing auth, Author Den, Nexet, Admin AI, notifications, editor, and database conventions documented. **Completed: see Section 4.**
...
- [x] Canonical Nexet/Author Den identity established. **Completed: Clerk user ID is canonical for server collaboration; Solo Author Den remains local-first by design.**
...
### Phase 5 — Contract lock and synchronized Nexet project
...
### Phase 6 — Ongoing Nexet permissions and supporting pages
...
- [x] Solo/Nexet Work navigation implemented. **Completed: `/authors/work` links Solo Work and Nexet Projects; `/authors/work/nexets` lists shared rooms for both participants.**
- [x] Nexet detail, waiting room, contract, Story Bible, messaging, and export surfaces implemented. **Completed: project detail with blocks/export, waiting room, contract room, story bible page, project activity page, and a Messages tab linked to the private thread when one exists.**
...
| 2026-08-14 | Phase 2 / Pitch Board | Server-backed seed discovery, URL filters, project posting, read-only detail, and closed/full handling verified | `pnpm --filter @workspace/nexet run typecheck`; `pnpm run typecheck:libs` |
...
| 2026-08-15 | Phase 6 / Nexet pages | Data-driven Atrium urgent cards; turn-aware project detail with read-only partner blocks and manuscript export; contract room; waiting room; Story Bible; project activity; Messages tab wired into routes | `pnpm run typecheck` |
...
| 2026-08-15 | Account activity + inbox fixes | New `GET /collaborations/activity` (account-wide, viewer-scoped feed across every room) and `GET /collaborations/threads` (inbox conversations with partner name, last-message preview, and unread state derived from unread message notifications). `/activity` page rebuilt from static placeholder into the real account trail; `/inbox` rebuilt as a structured two-panel inbox (Conversations + Room notes) with the collaboration inbox route redirecting to it. Seed creator can no longer answer their own seed in the UI (Server already 403'd; SeedDetail now shows “your own seed”, Pitch Board marks own posts). Follow-up: “Urgent work” (contract/turn/review/unread cards) moved off the Collaboration page onto the top of the `/inbox` page — the Atrium now keeps only wayfinding cards pointing to the inbox, trail, pitch board, review desk, and work | `pnpm --filter @workspace/api-server test` (32/32); `pnpm run typecheck`; live checks — Ada feed 14 events across 11 event types, threads list with partner/preview/unread, both roles scoped correctly; dev server serves the rebuilt inbox + slimmed Atrium |
| 2026-08-15 | Nexet UI polish (4 nav pages only) | Unified the four Nexet nav pages (Atrium `/dashboard`, Collaboration `/authors/atrium`, Activity `/activity`, Inbox `/inbox`) onto one header pattern — same `max-w-[1180px]` container, same `leading-[.86]` display heading, same `border-b-2` divider + right-side intro column. Fixed the Atrium dashboard's six-door grid: off-palette plum/blue/gold were replaced with the house palette (coral/teal/gold/ink + in-family shades), the single Available door is a filled coral card while Coming Soon doors read as paper “blueprints” with tinted icon chips, giving one clear focal point. Author Den untouched | `pnpm run typecheck`; `vite build` for nexet passes; dev server serves the updated modules |
| 2026-08-15 | Seed publishing + answering through Author Den (GitHub fork/PR model) | The pitch board's “Publish a seed” and “Answer this seed” now live inside Author Den instead of the Nexet `/respond` and `/authors/pitch-board/new` pages (both routes redirect). Publishing: seeds carry the full Author Den project snapshot (`project_document` jsonb on `collaboration_seeds`); “Post on Pitch Board” on a project card opens **The Brief** modal (What should a collaborator know?, Desired role, Respondent limit) before publishing; `?publish=1` opens the “A NEW ROOM FOR WORDS” new-project card. Answering: “Answer this seed” goes to `/authors-den/?answer=seedId`, opens an application, forks the frozen project into the respondent's studio as an editable project with a bright fork badge, and the clone card carries a **Submit** button that pops a “note for the creator” modal before sending the whole document (saved on the application + carried into the submission). Reviewing: the creator receives the submission in their inbox with a **Preview in Author Den** action; `/authors-den/?preview=continuationId` renders the fork read-only across General/Characters/Plots/World/Outline/Draft with **Approve & merge / Reject** buttons top-right. Accepting: `POST /continuations/{id}/accept` merges the fork over the seed document into a shared project (`document` jsonb on `collaboration_projects`), notifies both authors, closes the seed, and both studios materialize the shared project; participants keep it in sync via `GET/PUT /projects/{id}/document` (debounced push on edit + 20s pull). Respondents cannot re-apply to a seed with an unresolved submission (server 409 + seed detail shows “in review”; declined forks can resubmit with a fresh application). Author Den now authenticates through the same Clerk account (ClerkProvider in `main.tsx`, user identity shown on Settings) | `pnpm --filter @workspace/api-server test` (36/36, +4 fork/preview/accept/sync tests); `pnpm run typecheck` across workspace; both `vite build`s pass; live two-account checks 10/10 (publish w/ doc, fork-after-apply, submit carries doc, creator preview, accept merge, sync push/pull, re-apply 409, seed closed, notifications) against local Postgres |
| 2026-08-15 | Reading submitted projects moves to Author Den (no Nexet read pages) | Every entry point that reads a submitted write-up now opens the project directly in Author Den instead of a Nexet read page: the `continuation_submitted` inbox note now deep-links to `/authors-den/?preview=<continuationId>` (and the inbox card click hard-navigates there, covering older notes too); the review desk's “Read closely” became “Read in Author Den” (replacing the duplicate preview button) and the selection room's “Open read-only preview” became “Read in Author Den”; the private thread's “Back to submission” is now “Open submission in Author Den” shown only to the seed creator (the preview endpoint stays creator-scoped — respondents get 403). While previewing in Author Den the top-right actions are now clear **Approve (green) / Reject (red)** buttons that persist across the General, Character, Plot, World, Outline, and Draft views. | `pnpm --filter @workspace/api-server test` (36/36, deepLink assertion updated to `/authors-den/?preview=`); `pnpm run typecheck` across workspace; live two-account check — inbox note deepLink `/authors-den/?preview=…`, creator fetches preview doc, respondent 403; both dev servers serve the updated modules |
| 2026-08-15 | Shared “Author collaboration / private room” pages retire into Author Den | The legacy Nexet shared-room pages (`/authors/nexet/:id` turn-based blocks, `/contract`, `/waiting`, `/story-bible`, `/activity`) all redirect to `/authors-den/?project=<projectId>` — the shared project now lives in both studios, synced via the document push/pull. All entry points repointed: the inbox urgent cards (contract action, your turn, waiting on partner), the Nexet projects list cards, and the server notifications for `respondent_selected`, `contract_locked`, `contract_action_required`, `your_turn`, and `block_approved` now deep-link to `/authors-den/?project=…`. The legacy “Select” / “Select & open contract” buttons were removed from the review desk and selection room — the decision path is now Read in Author Den → **Approve & merge** (green) or **Reject** (red). Author Den surfaces a clear banner when a `?project=` link can't be materialized (e.g. a legacy room with no merged document yet). | `pnpm --filter @workspace/api-server test` (36/36); `pnpm run typecheck` across workspace; API bundle rebuilt + restarted; dev servers serve the redirect, the den fallback banner, and the repointed inbox/notification links |
| 2026-08-15 | Author Den sidebar hover-expand + wider pages + floating private thread | **Sidebar:** the rail now starts collapsed and auto-expands on hover (`onMouseEnter`/`onMouseLeave`); the PanelLeft toggle icon was removed from the “Authors Den / writing studio” brand row while keeping the hover trigger. **Layout:** `.page` max-width raised 1240→1580px, home 1270→1620px, editor/tone-rewrite layouts 1380→1580px so pages fill the stage instead of shrinking to the center. **Floating private thread:** shared/collaboration projects (after the creator accepts the fork) show a floating chat FAB carrying the co-writer's avatar; clicking expands a messages panel with send box, 15s refresh while open, and an unread badge poll when closed. Server: the accept transaction now auto-creates the thread (`projectView.threadId` resolves immediately), new `POST /collaborations/projects/{projectId}/thread` get-or-creates the room for either participant (403 for strangers), `threadView` now carries `projectId`/`creatorName`/`respondentName`, `collaboration_message` notifications deep-link to `/authors-den/?project=<id>&chat=1`, and the inbox threads list exposes `projectId`. Nexet inbox conversations deep-link into the Author Den project chat (`?project=&chat=1`) when the room is accepted, falling back to the Nexet thread only for pre-accept rooms. | `pnpm --filter @workspace/api-server test` (37/37, +1 private-thread/deep-link test); `pnpm run typecheck` across workspace; `vite build` for authors-den passes; live two-account check — accept auto-creates threadId, both roles get-or-create the same thread, message deepLink `/authors-den/?project=…&chat=1`, threads list exposes projectId; dev servers serve the chat widget, hover sidebar, wider pages, and repointed inbox |
| 2026-08-15 | Co-writer's real name in the private thread | Seeds now record the creator's display name at publish time: new nullable `creator_name` column on `collaboration_seeds` (applied to local Postgres + the in-memory test schema), the Author Den publish flow sends `creatorName` from the signed-in user (`user.fullName || username || firstName || project.author`), and the seed creation route stores it (falling back to “Author” for older seeds). `seedView` and `threadView` now surface `seed.creatorName ?? "Author"`, so the pitch board shows the real creator name and the floating chat shows the co-writer's real name on both sides (creator sees the respondent's name, respondent sees the creator's name) instead of a generic “Author”. | `pnpm --filter @workspace/api-server test` (37/37, thread test now asserts creatorName); `pnpm run typecheck` across workspace; live check — publish with `creatorName: Ada` → board shows Ada, thread creatorName Ada / respondentName Zoe |
| 2026-08-15 | Chat FAB badge driven by server unread state | The floating-chat unread badge is no longer a client-side “last opened” heuristic. New endpoints: `GET /collaborations/projects/{projectId}/thread/unread` returns `{ unread, count }` for the viewer derived from unread `collaboration_message` notifications (participants only, 403 for strangers, `{unread:false,count:0}` when no thread yet) and `POST /collaborations/projects/{projectId}/thread/read` marks the viewer's messages read (204). The Author Den polls the unread endpoint every 20s for the FAB badge, marks read when the chat opens and on each open-panel refresh, and drops the `chatSeenAtRef` timestamp entirely — so the badge and the Nexet inbox stay consistent with what the user has actually opened. Follow-up: collapsing the chat also calls the mark-read endpoint, so a partner reply that arrived while the panel was open never leaves a stale badge behind after collapse. | `pnpm --filter @workspace/api-server test` (37/37, private-thread test now covers unread→mark-read→cleared + stranger 403); `pnpm run typecheck` across workspace; live check — Ada message → Zoe unread `{true,1}`, mark-read 204 → `{false,0}`, inbox thread list agrees |
| 2026-08-15 | Incoming-message alert (toast + chime) for the private thread | When a new private-thread message arrives while the chat is collapsed, Author Den now pops a subtle **alert pill** above the chat FAB (partner avatar + “New message from …”, click to open, dismiss button, auto-dismisses after 5s) and plays a **soft two-pip chime** generated with the Web Audio API (no asset needed; silently skipped if audio is blocked). Detection is driven by the server unread poll: a 0→>0 transition while collapsed triggers the alert (the first poll for a project never chimes, so pre-existing unread messages only show on the badge). | `pnpm run typecheck` across workspace; dev server serves the alert pill, chime, and poll logic (no server or test changes) |
| 2026-08-15 | Legacy shared rooms open in Author Den instead of the “no merged document” error + Draft page fixes | **Legacy rooms:** rooms created before the fork/merge model (work blocks + story bible, but no `document` column — e.g. older contract/turn rooms) now report `documentAvailable: true` and `GET /projects/{id}/document` synthesizes an Author Den project document from the room's approved work blocks + shared story bible entries, so `?project=` links open them in the studio instead of dead-ending on the “This shared room has no merged document” banner. **Draft page:** the editor text area is wider (draft workspace max-width 1500→1700px, tone column 350→410px, draft-area side padding capped at 120px) and the TONE MATCH card is wider; the TONE MATCH heading no longer carries the “Select text in the draft…” description or the “Selected text / privacy first” chip. **Editor toolbar:** all formatting buttons (bold/italic/headings/lists/quote/link/clear/code + undo/redo) now prevent `mousedown` default so focus and the text selection stay inside the editor when a button is clicked — `execCommand` now applies to the selected text instead of silently no-oping; the editor is focused before each command runs. | `pnpm --filter @workspace/api-server test` (38/38, +1 legacy-room synthesized-document test); `pnpm run typecheck` across workspace; `vite build` for authors-den passes; API bundle rebuilt + restarted; live checks — `documentAvailable` + synthesized doc on real rooms, den serves the new modules and CSS |
| 2026-08-15 | Oracle admin integrated as a standalone artifact at `/oracle-admin/` | The Story Oracle admin page is a separate control room (per the Author Studio architecture) and does **not** live in the Nexet member shell or the Author Den nav. It was integrated from the `comp_last` repository's `artifacts/oracle-admin` as its own workspace package at `artifacts/oracle-admin`, served on its own port under **`/oracle-admin/`** (dev: `PORT=5175 BASE_PATH=/oracle-admin/ npx vite`). It reuses the existing `@workspace/api-client-react` admin hooks + admin API in `artifacts/api-server/src/routes/admin.ts` (session/login/logout, providers list/update, check) and the Oracle routes in `routes/oracle.ts` (chat/continuity/rewrite/outline/voice/world-bible). The admin surface has a login gate (access code default **NEXET_123**, overridable via `ADMIN_ACCESS_CODE`), a system overview (healthy/configured/failover metrics, provider health table, run live probe via `oracleChat`), and a provider-routing desk (Groq/OpenRouter/Ollama/LM Studio: API credential, base URL, model id, priority, enable/disable, save, test connection, status pills). A `/api` dev proxy was added to its `vite.config.ts` (mirrors the production router). **Undo note:** the earlier Nexet `/admin` page and its Profile link were removed — the admin is intentionally separate from Nexet navigation. **.env credentials** (kept): the API server loads `.env` on boot via `src/env.ts`; `.env.example` documents `ADMIN_ACCESS_CODE`, `SESSION_SECRET`, and per-provider credentials; `ensureProviderRows` seeds provider keys/base URLs/models from those env vars when rows have none. | `pnpm --filter @workspace/api-server test` (41/41, incl. 3 admin tests); `pnpm run typecheck` across workspace; oracle-admin `vite build` passes (1770 modules); live checks — session → login `NEXET_123` → authenticated → providers list all respond through the dev proxy; dev server serves the app at `http://localhost:5175/oracle-admin/` |
| 2026-08-15 | Freebuff model provider in the Story Oracle | Added **Freebuff** as a fifth Oracle provider (alongside Groq/OpenRouter/Ollama/LM Studio). `oracle.ts`: `ProviderId` union extended, `providerDefinitions` gains `freebuff` (label “Freebuff”, default base URL `http://localhost:8081/v1`, default model `deepseek-v4-flash`), `providerEnv` maps `FREEBUFF_API_KEY` / `FREEBUFF_BASE_URL` / `FREEBUFF_MODEL_ID`, and `endpointFor` now tolerates a base URL that already ends in `/chat/completions`. OpenAPI `ProviderId` enum extended to include `freebuff` and the clients (api-client-react + api-zod) were regenerated. The oracle-admin UI gained the Freebuff card (provider meta, “FB” mark, free-form model id + priority + enabled controls) so the model is chosen in the admin page. `.env.example` documents the three Freebuff vars; the API key can be entered in the Oracle admin page (encrypted at rest) or via `.env`. | `pnpm --filter @workspace/api-server test` (41/41, provider-list test now expects 5 providers); `pnpm run typecheck` across workspace; oracle-admin `vite build` passes; live checks — login → providers list shows `freebuff` with its models, check endpoint responds gracefully when unconfigured, PUT saves+encrypts a key (keyHint `••••••••9999`, cleared afterward); API rebuilt + restarted |
| 2026-08-15 | Freebuff model dropdown in the Oracle admin | The Freebuff provider now ships Freebuff's full published model catalog (DeepSeek V4 Flash 07/31, DeepSeek V4 Pro 08/13, GPT-5.6 Luna, MiniMax M3, MiMo 2.5, GLM 5.2, Gemini 3.1 Flash Lite) and the admin page's model field is a **dropdown** listing them, with a “Custom model…” option that reveals a text input for ids not in the catalog. The server appends a saved custom model to the status list (`(custom)` label) so it stays visible/selected after save; the other providers' cards get the same dropdown treatment. | `pnpm --filter @workspace/api-server test` (41/41); `pnpm run typecheck` across workspace; oracle-admin `vite build` passes; live check — providers list shows all 7 freebuff models; dev server serves the dropdown |
