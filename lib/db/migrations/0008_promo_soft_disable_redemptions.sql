...
ALTER TABLE nexet_promo_codes ADD COLUMN active boolean NOT NULL DEFAULT true;

-- 2. One row per (code, user): shared codes are usable by many people, but
--    each person can redeem a given code only once.
CREATE TABLE IF NOT EXISTS nexet_promo_redemptions (
  code TEXT NOT NULL REFERENCES nexet_promo_codes(code) ON DELETE CASCADE,
