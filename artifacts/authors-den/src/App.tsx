...
function App() {
  ...
  const exportFile = (format: "json" | "md" | "txt" | "html") => { if (!project) return; const scenes = project.scenes.map((scene) => ({ ...scene, content: stripHtml(scene.content) })); const body = format === "json" ? JSON.stringify({ ...project, scenes }, null, 2) : format === "md" ? `# ${project.title}\n\n_${project.author}_\n\n## Premise\n${project.premise}\n\n${scenes.map((scene) => `## ${scene.title}\n\n${scene.content}`).join("\n\n")}` : format === "html" ? `<!doctype html><title>${project.title}</title><h1>${project.title}</h1>${scenes.map((scene) => `<h2>${scene.title}</h2><p>${scene.content.replace(/\n\n/g, "</p><p>")}</p>`).join("")}` : `${project.title}\n${project.author}\n\n${scenes.map((scene) => `${scene.title}\n\n${scene.content}`).join("\n\n")}`; const link = document.createElement("a"); link.href = URL.createObjectURL(new Blob([body], { type: "text/plain" })); link.download = `${project.title.replace(/\s+/g, "-").toLowerCase()}.${format === "json" ? "msk.json" : format}`; link.click(); URL.revokeObjectURL(link.href); notify(`Downloaded ${format.toUpperCase()}`); };
...
function Sidebar({ view, setView, project, projects, openProject, openEditor, mobile, close, onNew, onTutorial, workspaceOpen, setWorkspaceOpen, mode, hasUserProject, notify, collapsed, setCollapsed }: { view: View; setView: (view: View) => void; project?: Project; projects: Project[]; openProject: (project: Project, view?: View, sceneId?: string) => void; openEditor: (sceneId?: string) => void; mobile: boolean; close: () => void; onNew: () => void; onTutorial: () => void; workspaceOpen: boolean; setWorkspaceOpen: (open: boolean) => void; mode: "lesson" | "draft"; hasUserProject: boolean; notify: (message: string) => void; collapsed: boolean; setCollapsed: (collapsed: boolean) => void }) {
  const nav: [View, string, ReactNode][] = [["general", "General", <PenLine size={17} />], ["characters", "Characters", <Users size={17} />], ["plots", "Plots", <Sparkles size={17} />], ["world", "World", <Globe2 size={17} />], ["outline", "Outline", <ClipboardList size={17} />], ["editor", "Draft", <BookOpen size={17} />]];
