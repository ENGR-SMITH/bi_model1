...
// started from anywhere the project limit bites). After Whop redirects the
// customer back, the URL carries ?reference=…; this gate confirms the charge
// with the server, refreshes the project quota, and shows the outcome.
// ---------------------------------------------------------------------------
      ...
      <div className="modal small-modal plan-modal" role="dialog" aria-modal="true" data-testid="whop-return-gate" onClick={(event) => event.stopPropagation()}>
        ...
        {state.kind === "busy" && (
          <>
            ...
            <p className="profile-footnote mt-3" style={{ display: "flex", gap: 6 }}><Loader2 size={13} className="spin" /> Checking with Whop…</p>
          </>
        )}
        {state.kind === "success" && (
          <>
