...
// ---------------------------------------------------------------------------
// PaystackReturnGate — mounted once at the studio root (buy-more projects can
// be started from anywhere the project limit bites). After Paystack redirects
// the customer back, the URL carries ?reference=…; this gate confirms the
// charge with the server, refreshes the project quota, and shows the outcome.
// ---------------------------------------------------------------------------
      ...
      <div className="modal small-modal plan-modal" role="dialog" aria-modal="true" data-testid="paystack-return-gate">
        ...
        {state.kind === "busy" && (
          <>
            ...
            <p>This can take a few seconds. Your project limit updates as soon as it lands.</p>
            <p className="profile-footnote mt-3" style={{ display: "flex", gap: 6 }}><Loader2 size={13} className="spin" /> Checking with Paystack…</p>
          </>
        )}
        {state.kind === "success" && (
          <>
            ...
            <p>
              {state.total !== undefined ? (
                <>Charged ${(state.total / 100).toFixed(2)}{state.cardLast4 ? <> · card •••• {state.cardLast4}</> : null} — the bar now shows your new limit.</>
              ) : (
                <>Your project limit has been extended.</>
              )}
            </p>
