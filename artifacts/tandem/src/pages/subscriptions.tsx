  ...
  return (
    <div className="mx-auto max-w-[1060px]">
      ...
      {plansQuery.isLoading ? (
        <div className="public-card mt-10 rounded-[1.5rem] border-2 border-white/10 bg-[#111111] p-8 text-center text-zinc-500">Opening the price list…</div>
      ) : (
    ...
    <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-[#111111]/60 p-4 backdrop-blur-sm" data-testid="subscription-pay-gate">
      <div className="relative w-full max-w-md rounded-3xl border-2 border-dashed border-[#e1b956] bg-[#111111] text-white shadow-[12px_14px_0_rgba(41,43,69,0.18)]">
        <div className="rounded-t-[1.35rem] p-6 pb-5">
          ...
          <button type="button" onClick={onClose} aria-label="Close" className="focus-house absolute right-4 top-4 rounded-full p-1.5 text-zinc-500 hover:bg-[#d6cbb9]/40 hover:text-white"><X className="h-4 w-4" /></button>
        </div>
        ...
        {result ? (
          <div className="rounded-b-[1.35rem] p-6 pt-5" data-testid="subscription-success">
            ...
            <button type="button" onClick={onPaid} className="focus-house mt-5 w-full rounded-xl bg-[#111111] py-3.5 text-sm font-bold text-[#fff4e6] transition-colors hover:bg-[#3e8074]">Done</button>
          </div>
        ) : (
          <div className="rounded-b-[1.35rem] p-6 pt-5">
            <label className="block">
              ...
              <div className="relative mt-2">
                ...
                <input value={cardNumber} onChange={(e) => setCardNumber(formatCardNumber(e.target.value))} placeholder="4242 4242 4242 4242" inputMode="numeric" autoComplete="cc-number" className="focus-house w-full rounded-xl border-2 border-white/10 bg-[#111111] py-3 pl-11 pr-4 text-sm text-white placeholder:text-zinc-600" data-testid="sub-input-card" />
              </div>
            </label>
            <div className="mt-3 grid grid-cols-2 gap-3">
              <label className="block">
                <span className="font-mono-ui text-[10px] uppercase tracking-[0.16em] text-zinc-500">Expiry</span>
                <input value={expiry} onChange={(e) => setExpiry(formatExpiry(e.target.value))} placeholder="MM/YY" inputMode="numeric" autoComplete="cc-exp" className="focus-house mt-2 w-full rounded-xl border-2 border-white/10 bg-[#111111] py-3 px-4 text-sm text-white placeholder:text-zinc-600" data-testid="sub-input-expiry" />
              </label>
              <label className="block">
                <span className="font-mono-ui text-[10px] uppercase tracking-[0.16em] text-zinc-500">CVC</span>
                <input value={cvc} onChange={(e) => setCvc(e.target.value.replace(/\D/g, '').slice(0, 4))} placeholder="123" inputMode="numeric" autoComplete="cc-csc" className="focus-house mt-2 w-full rounded-xl border-2 border-white/10 bg-[#111111] py-3 px-4 text-sm text-white placeholder:text-zinc-600" data-testid="sub-input-cvc" />
              </label>
            </div>
            <div className="mt-4">
              <span className="font-mono-ui text-[10px] uppercase tracking-[0.16em] text-zinc-500">Promo code</span>
              <input value={promo} onChange={(e) => setPromo(e.target.value.toUpperCase())} placeholder="PROMOCODE" className="focus-house mt-2 w-full rounded-xl border-2 border-white/10 bg-[#111111] px-4 py-3 text-sm uppercase tracking-[0.1em] text-white placeholder:text-zinc-600" data-testid="sub-input-promo" />
            </div>
            ...
            <button type="button" onClick={pay} disabled={purchase.isPending} className="focus-house mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-[#111111] py-3.5 text-sm font-bold text-[#fff4e6] transition-colors hover:bg-[#3e8074] disabled:cursor-wait disabled:opacity-60" data-testid="sub-button-pay">
