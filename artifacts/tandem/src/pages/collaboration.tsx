...
function Tandems() { const q = useListCollaborationProjects(); const ps: any[] = q.data || []; return <Frame title="Tandem projects" intro="The work after the yes. Shared rooms live in both Author Den studios, kept in sync between the two of you.">{q.isLoading ? <Loading /> : q.isError ? <ErrorState retry={q.refetch} /> : ps.length ? <div className="mt-10 grid gap-5 md:grid-cols-2">{ps.map(p => <button key={p.id} data-testid={`tandem-project-${p.id}`} onClick={() => window.location.assign(`/authors-den/?project=${p.id}`)} className="soft-lift focus-house rounded-[1.5rem] border-2 border-[#d6cbb9] bg-[#fff4e6] p-7 text-left"><Pill>{p.status}</Pill><h2 className="mt-7 font-display text-3xl italic">{p.title}</h2><p className="mt-2 text-sm text-[#77717a]">{p.creatorName} and {p.respondentName}</p><div className="mt-8 flex items-center justify-between border-t border-[#d6cbb9] pt-4 text-xs text-[#77717a]"><span>Open in Author Den</span><ArrowRight className="h-4 w-4 text-[#e55b4c]" /></div></button>)}</div> : <Empty title="No shared rooms yet." body="When a creator accepts a submission, the merged project settles into both studios — kept in sync as you write together." href="/authors/pitch-board" action="Find a seed" />}</Frame>; }
