...
function ErrorState({ retry }: { retry?: () => void }) { return <div className="mt-10 rounded-[1.5rem] border-2 border-[#3b82f6]/40 bg-[#111111] p-8"><CircleAlert className="text-[#3b82f6]" /><p className="mt-4 font-display text-3xl italic">The room is having trouble opening.</p><p className="mt-2 text-sm text-zinc-500">Your work is safe. Try the door again in a moment.</p>{retry && <button onClick={retry} className="focus-house mt-5 rounded-full bg-[#111111] px-5 py-3 text-sm font-bold text-[#fff4e6]">Try again</button>}</div>; }
function Empty({ title, body, href, action }: { title: string; body: string; href?: string; action?: string }) { return <div className="mt-10 rounded-[1.75rem] border-2 border-white/10 bg-[#111111] p-8 shadow-[8px_10px_0_rgba(41,43,69,.07)] sm:p-10"><Sparkles className="h-7 w-7 text-[#3b82f6]" /><p className="mt-7 font-display text-4xl italic">{title}</p><p className="mt-3 max-w-xl text-sm leading-[1.8] text-zinc-500">{body}</p>{href && <Link href={href} className="focus-house mt-7 inline-flex items-center gap-2 rounded-full bg-[#111111] px-5 py-3 text-sm font-bold text-[#fff4e6]">{action}<ArrowRight className="h-4 w-4" /></Link>}</div>; }
...
function SeedCard({ seed, isOwn }: { seed: any; isOwn?: boolean }) { return <Link href={`/authors/pitch-board/seed/${seed.id}`} className="soft-lift focus-house group block rounded-[1.5rem] border-2 border-white/10 bg-[#111111] p-6"><div className="flex items-start justify-between gap-4"><Pill>{seed.genre || 'Open brief'}</Pill><span className="font-mono-ui text-[10px] text-zinc-600">{seed.respondentCount}/{seed.respondentLimit || '∞'} voices</span></div><h2 className="mt-7 font-display text-3xl italic leading-none">{seed.sourceProjectTitle}</h2><p className="mt-4 line-clamp-3 text-sm leading-[1.8] text-zinc-400">{seed.seedText}</p><div className="mt-7 flex items-center justify-between border-t border-white/10 pt-4 text-xs text-zinc-500">{isOwn ? <span className="rounded-full bg-[#f0c85c] px-2.5 py-1 font-mono-ui text-[9px] uppercase tracking-[.12em] text-white">Your seed</span> : <span>{seed.creatorName} · {seed.tone}</span>}<ArrowUpRight className="h-4 w-4 text-[#3b82f6] transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" /></div></Link>; }
function CreateSeed() {
  ...
  return <Frame eyebrow="Pitch board / publish" title="Leave the door open." intro="Publish a frozen writing seed and a clear brief. Your original project stays yours; this post becomes its own collaboration invitation.">
    <div className="reveal reveal-1 mt-10 grid gap-6 lg:grid-cols-[1fr_.8fr]">
      <section className="rounded-[1.5rem] border-2 border-white/10 bg-[#111111] p-7 sm:p-9">
...
function Board() {
  ...
  return <Frame title="The pitch board" intro="Frozen seeds from writers making room for another mind. Filter gently; the right opening is often a quiet one.">
    <div className="mt-8 flex flex-wrap gap-3">
      ...
      <button onClick={() => { window.location.href = '/authors-den/?publish=1'; }} className="focus-house inline-flex items-center gap-2 rounded-full bg-[#3b82f6] px-4 py-3 text-sm font-bold text-[#fff4e6]"><PenLine className="h-4 w-4" />Publish a seed</button>
...
function Respond() {
  ...
  return <Frame eyebrow="Pitch board / response" title={locked ? "Your continuation is in review." : "Make an opening."} intro={locked ? "This submission is locked while the creator reviews it. The frozen seed remains read-only." : "This is a private draft. Nothing leaves your desk until you choose to submit it."}>
    <div className="reveal reveal-1 mt-10 grid gap-6 lg:grid-cols-[.7fr_1.3fr]">
      <div className="rounded-[1.5rem] bg-[#111111] p-7 text-[#fff4e6]"><BookOpen className="text-[#f0c85c]" /><p className="mt-8 font-mono-ui text-[10px] uppercase tracking-[.18em] text-[#f0c85c]">Private clone · source seed</p><p className="mt-3 font-display text-3xl italic">{seedQ.data?.sourceProjectTitle || 'Loading seed…'}</p><p className="mt-5 whitespace-pre-wrap text-sm leading-[1.8] text-[#d9d2cb]">{seedQ.data?.seedText}</p><div className="mt-8 border-t border-[#fff4e6]/20 pt-4 text-xs text-[#d9d2cb]">The seed is frozen. Only your continuation and note can change.</div></div>
      <div className="rounded-[1.5rem] border-2 border-white/10 bg-[#111111] p-7">
  ...
  return <Frame title="The review desk" intro="A continuation is a generous thing to receive. Read each one slowly, then choose with clarity.">
    <div className="mt-10 space-y-4">{q.isLoading ? <Loading /> : q.isError ? <ErrorState retry={q.refetch} /> : rows.length ? rows.map(c =>
      <article key={c.id} data-testid={`continuation-row-${c.id}`} className={`soft-lift rounded-[1.5rem] border-2 p-6 ${unread.has(c.id) || !c.read ? 'border-[#3b82f6]/50 bg-[#111111]' : 'border-white/10 bg-[#111111]'}`}>
        <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex flex-wrap gap-2"><Pill>{c.status.replaceAll('_', ' ')}</Pill>{(!c.read || unread.has(c.id)) && <span className="rounded-full bg-[#3b82f6] px-2.5 py-1 font-mono-ui text-[9px] uppercase tracking-[.12em] text-[#fff4e6]">New</span>}</div><h2 className="mt-4 font-display text-3xl italic">{c.sourceProjectTitle}</h2><p className="mt-1 text-sm text-zinc-500">From {c.respondentName} · submitted {new Date(c.submittedAt).toLocaleDateString()}</p></div><button data-testid={`link-read-continuation-${c.id}`} disabled={c.status !== 'UNDER_REVIEW'} onClick={() => window.location.assign(`/authors-den/?preview=${c.id}`)} className="focus-house inline-flex items-center gap-1 rounded-full border-2 border-[#3e8074] px-4 py-2 text-sm font-bold text-[#2f675e] disabled:cursor-not-allowed disabled:opacity-40">Read in Author Den <ArrowRight className="ml-1 inline h-4 w-4" /></button></div>
  ...
  return <Frame eyebrow="Review desk / selection room" title="Choose the next voice." intro="Compare submitted continuations without importing any respondent project into your Studio. Selection remains yours; advisory signals never decide for you.">
    <div className="mt-8 flex flex-wrap items-center gap-3">
      <div className="flex rounded-full border-2 border-white/10 bg-[#111111] p-1" role="tablist" aria-label="Selection views">
        {([['text', 'Their text'], ['comments', 'Their comments'], ['voice', 'Voice notes']] as const).map(([id, label]) => <button key={id} role="tab" aria-selected={view === id} onClick={() => setView(id)} className={`rounded-full px-4 py-2 text-xs font-bold ${view === id ? 'bg-[#111111] text-[#fff4e6]' : 'text-zinc-500'}`}>{label}</button>)}
      </div>
  ...
  return <Frame eyebrow="Review desk / read only" title={c.sourceProjectTitle} intro={`A response from ${c.respondentName}. The seed and its answer stay side by side. Nothing here changes the submitted text.`}>
    <div className="mt-10 grid gap-5 lg:grid-cols-[1.1fr_.9fr]"><div className="space-y-5"><div className="rounded-[1.5rem] bg-[#f0c85c] p-8"><Pill>Frozen seed</Pill><p data-testid="text-frozen-seed" className="mt-8 whitespace-pre-wrap font-display text-2xl leading-[1.35]">{c.seedText}</p></div><div className="rounded-[1.5rem] bg-[#111111] p-8 shadow-[8px_10px_0_rgba(41,43,69,.07)]"><Pill>Continuation</Pill><p data-testid="text-continuation" onMouseUp={handleSelect} className="mt-8 select-text whitespace-pre-wrap font-display text-2xl leading-[1.35]"><AnnotatedText text={c.continuationText} annotations={annotations} /></p><p className="mt-3 text-xs text-zinc-500">Select any passage to annotate it. Annotations are anchored to the submitted text and never edit it.</p>{sel && <div className="mt-4 rounded-xl border-2 border-[#f0c85c] bg-[#111111] p-4"><p className="font-mono-ui text-[10px] uppercase tracking-[.18em] text-[#3b82f6]">Annotate characters {sel.start}–{sel.end}</p><textarea value={annBody} onChange={e => setAnnBody(e.target.value)} placeholder="A note for the writer about this passage…" className="mt-3 min-h-[70px] w-full rounded-xl border border-white/10 bg-[#111111] p-3 text-sm outline-none focus:border-[#3b82f6]" /><div className="mt-3 flex gap-2"><button data-testid="button-save-annotation" onClick={saveAnnotation} disabled={createAnnotation.isPending || !annBody.trim()} className="focus-house rounded-full bg-[#111111] px-4 py-2 text-xs font-bold text-[#fff4e6] disabled:opacity-40">{createAnnotation.isPending ? 'Saving…' : 'Save annotation'}</button><button onClick={() => { setSel(null); setAnnBody(''); }} className="focus-house rounded-full border border-white/10 px-4 py-2 text-xs font-bold text-zinc-500">Cancel</button></div></div>}<div className="mt-6 space-y-3">{annotations.length ? annotations.map(ann => <div key={ann.id} data-testid={`annotation-${ann.id}`} className="rounded-xl border border-white/10 bg-[#111111] p-4 text-sm"><p className="font-bold">{authorLabel(ann.authorId)} <span className="font-normal text-zinc-500">· characters {ann.rangeStart}–{ann.rangeEnd}</span></p><p className="mt-1 leading-relaxed text-zinc-400">“{c.continuationText.slice(ann.rangeStart, ann.rangeEnd)}”</p><p className="mt-2 text-white">{ann.body}</p></div>) : <p className="text-xs text-zinc-600">No annotations yet.</p>}</div><div className="mt-8 border-t border-white/10 pt-5 text-sm leading-relaxed text-zinc-400"><strong className="text-white">A note from the writer</strong><p className="mt-2">{c.comments || 'No separate note was left.'}</p></div></div></div>
      <aside className="space-y-5"><section className="rounded-[1.5rem] border-2 border-white/10 bg-[#111111] p-6"><div className="flex items-center gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-full bg-[#3e8074] text-lg font-bold text-[#fff4e6]">{(p?.displayName || c.respondentName || 'W').slice(0,1)}</span><div><p className="font-bold">{p?.displayName || c.respondentName}</p><p className="text-xs text-zinc-500">Writer profile</p></div></div>{p ? <><p className="mt-5 text-sm leading-relaxed text-zinc-400">{p.bio || 'This writer has not added a biography yet.'}</p><div className="mt-5 flex flex-wrap gap-2">{[...(p.genres || []), ...(p.tones || [])].map((x: string) => <Pill key={x}>{x}</Pill>)}</div><dl className="mt-6 grid grid-cols-3 gap-2 border-t border-white/10 pt-5 text-center"><div><dt className="font-mono-ui text-[9px] uppercase text-zinc-500">Sent</dt><dd className="mt-1 text-xl font-bold">{p.submittedCount}</dd></div><div><dt className="font-mono-ui text-[9px] uppercase text-zinc-500">Accepted</dt><dd className="mt-1 text-xl font-bold">{p.acceptedCount}</dd></div><div><dt className="font-mono-ui text-[9px] uppercase text-zinc-500">Completed</dt><dd className="mt-1 text-xl font-bold">{p.completedCount}</dd></div></dl></> : <div className="mt-5 h-20 animate-pulse rounded-xl bg-[#111111]" />}</section>
        ...
        <section className="rounded-[1.5rem] border-2 border-white/10 bg-[#111111] p-6"><div className="flex items-center gap-3"><MessageCircle className="text-[#3b82f6]" /><div><p className="font-bold">Private conversation</p><p className="text-xs text-zinc-500">Only the two people in this room can read it.</p></div></div><button data-testid="button-open-thread" onClick={() => existingThread ? setLocation(`/authors/collaborations/thread/${existingThread.id}`) : openThread()} disabled={start.isPending} className="focus-house mt-5 w-full rounded-full bg-[#111111] px-4 py-3 text-sm font-bold text-[#fff4e6]">{start.isPending ? 'Opening…' : existingThread ? 'Open private thread' : 'Start private thread'}</button></section>
...
function ProjectTabs({ projectId, active, threadId }: { projectId: string; active: string; threadId?: string | null }) {
  ...
  return <nav className="mt-8 flex flex-wrap gap-2" aria-label="Project pages">{tabs.map(([suffix, label]) => <Link key={suffix} href={suffix === 'messages' ? `/authors/collaborations/thread/${threadId}` : `${base}${suffix}`} aria-current={active === suffix ? 'page' : undefined} className={`focus-house rounded-full px-4 py-2 text-xs font-bold ${active === suffix ? 'bg-[#111111] text-[#fff4e6]' : 'border-2 border-white/10 bg-[#111111] text-zinc-500'}`}>{label}</Link>)}</nav>;
}
  ...
  return <Frame title={p.title} intro={`${p.creatorName} and ${p.respondentName} are carrying this piece together.`}>
    ...
    <div className="reveal reveal-1 mt-8 grid gap-5 lg:grid-cols-[1fr_300px]">
      ...
      <aside className="h-fit space-y-5">
        ...
        <section className="rounded-[1.5rem] border-2 border-white/10 bg-[#111111] p-6"><p className="font-mono-ui text-[10px] uppercase tracking-[.18em] text-[#3b82f6]">Attribution trail</p><p className="mt-3 text-xs leading-relaxed text-zinc-500">Who wrote what, and what each pass continues from. Never editable, always present.</p>{genealogyQ.isLoading ? <div className="mt-4 h-16 animate-pulse rounded-xl bg-[#111111]" /> : <ol className="mt-4 space-y-2.5">{(genealogyQ.data || []).length ? (genealogyQ.data as any[]).map((g) => <li key={g.id} className="flex items-center gap-2 text-xs"><span className="h-1.5 w-1.5 shrink-0 rounded-full bg-[#3e8074]" /><span className="font-bold">{g.contributorName}</span><span className="text-zinc-500">— {kindLabel(g.kind)}</span>{g.parentBlockId && <span className="text-zinc-600">· continues a pass</span>}</li>) : <li className="text-xs text-zinc-500">Nothing recorded yet.</li>}</ol>}</section>
      </aside>
    </div>
  </Frame>;
}
...
function ProjectActivityPage() {
  ...
  return <Frame eyebrow="Tandem / activity" title="A clear record." intro="Who did what, when. Summaries only — hidden prose never enters this log."><ProjectTabs projectId={projectId} active="/activity" /><div className="reveal reveal-1 mt-8 max-w-3xl space-y-3">{q.isLoading ? <Loading /> : q.isError ? <ErrorState retry={q.refetch} /> : events.length ? events.map((event) => <div key={event.id} data-testid={`activity-${event.id}`} className="flex items-start gap-4 rounded-[1.25rem] border-2 border-white/10 bg-[#111111] p-5"><History className="mt-0.5 h-4 w-4 shrink-0 text-[#3b82f6]" /><div><p className="text-sm font-bold">{event.summary}</p><span className="mt-1 block font-mono-ui text-[9px] uppercase tracking-[.12em] text-zinc-600">{event.eventType.replaceAll('_', ' ')} · {new Date(event.createdAt).toLocaleString()}</span></div></div>) : <Empty title="Nothing recorded yet." body="Contract approvals, submitted passes, and shared bible notes will appear here as the room moves." />}</div></Frame>;
}
