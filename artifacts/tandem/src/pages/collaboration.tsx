...
import {
  ...
  useApproveCollaborationContract, useApproveCollaborationWorkBlock, useCreateCollaborationSeed, useCreateCollaborationStoryBibleEntry, useCreateCollaborationWorkBlock, useCreateSeedApplication, useDeclineContinuation, useGetCollaborationInbox,
...
function SeedCard({ seed }: { seed: any }) { return <Link href={`/authors/pitch-board/seed/${seed.id}`} className="soft-lift focus-house group block rounded-[1.5rem] border-2 border-[#d6cbb9] bg-[#fff4e6] p-6"><div className="flex items-start justify-between gap-4"><Pill>{seed.genre || 'Open brief'}</Pill><span className="font-mono-ui text-[10px] text-[#98909a]">{seed.respondentCount}/{seed.respondentLimit || '∞'} voices</span></div><h2 className="mt-7 font-display text-3xl italic leading-none">{seed.sourceProjectTitle}</h2><p className="mt-4 line-clamp-3 text-sm leading-[1.8] text-[#625f6d]">{seed.seedText}</p><div className="mt-7 flex items-center justify-between border-t border-[#d6cbb9] pt-4 text-xs text-[#77717a]"><span>{seed.creatorName} · {seed.tone}</span><ArrowUpRight className="h-4 w-4 text-[#e55b4c] transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" /></div></Link>; }
function Atrium() {
  const { user } = useUser();
  ...
  const notifications: any[] = inboxQ.data || [];
  const continuations: any[] = continuationsQ.data || [];
  const unread = notifications.filter((note: any) => !note.read);
  const pendingReviews = continuations.filter((c: any) => c.status === 'UNDER_REVIEW');
  const urgent: Array<{ kind: string; label: string; body: string; href: string; icon: ReactNode }> = [];
  projects.forEach((p: any) => {
    ...
    if (p.status === 'CONTRACT_PENDING' && !myApproved) {
      urgent.push({ kind: 'contract', label: 'Contract action required', body: `Approve the contract for “${p.title}” so the room can open.`, href: `/authors/tandem/${p.id}/contract`, icon: <FileText /> });
    } else if (p.status === 'ACTIVE' && p.currentTurn === myRole) {
      urgent.push({ kind: 'turn', label: 'Your turn', body: `The next pass in “${p.title}” is yours to write.`, href: `/authors/tandem/${p.id}`, icon: <PenLine /> });
    } else if (p.status === 'ACTIVE') {
      urgent.push({ kind: 'waiting', label: 'Waiting on partner', body: `${roleLabel(p, p.currentTurn)} is carrying “${p.title}”.`, href: `/authors/tandem/${p.id}/waiting`, icon: <Hourglass /> });
    }
  });
  if (pendingReviews.length) {
    urgent.push({ kind: 'review', label: 'Review pending', body: `${pendingReviews.length} continuation${pendingReviews.length === 1 ? '' : 's'} waiting on your eye.`, href: '/authors/collaborations/continuations', icon: <Inbox /> });
  }
  if (unread.length) {
    urgent.push({ kind: 'inbox', label: `${unread.length} unread note${unread.length === 1 ? '' : 's'}`, body: 'Private notifications from your rooms.', href: '/authors/collaborations/inbox', icon: <MessageCircle /> });
  }
  ...
  return <Frame title="Two voices. One room." intro="A considered way to find the right creative counterpart. Seeds stay frozen; the work that follows stays visible.">
    {urgent.length > 0 && <section className="reveal reveal-1 mt-10"><div className="flex items-center justify-between gap-4"><h2 className="font-mono-ui text-[10px] uppercase tracking-[.2em] text-[#e55b4c]">Urgent work</h2><span className="font-mono-ui text-[10px] uppercase tracking-[.12em] text-[#98909a]">{urgent.length} item{urgent.length === 1 ? '' : 's'} need you</span></div><div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{urgent.slice(0, 6).map((item) => <Link key={`${item.kind}-${item.href}`} href={item.href} className={`soft-lift focus-house rounded-[1.25rem] p-6 ${cardClass(item.kind)}`}><span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#292b45]/10">{item.icon}</span><h3 className="mt-6 text-lg font-extrabold tracking-[-.03em]">{item.label}</h3><p className="mt-1 text-xs leading-relaxed opacity-80">{item.body}</p><span className="mt-5 inline-flex items-center gap-2 text-xs font-bold">Open <ArrowRight className="h-3.5 w-3.5" /></span></Link>)}</div></section>}
    <div className="reveal reveal-1 mt-10 grid gap-5 md:grid-cols-[1.2fr_.8fr]"><Link href="/authors/pitch-board" className="soft-lift focus-house rounded-[1.75rem] bg-[#292b45] p-8 text-[#fff4e6] sm:p-10"><Search className="h-7 w-7 text-[#f0c85c]" /><p className="mt-16 font-mono-ui text-[10px] uppercase tracking-[.2em] text-[#f0c85c]">The pitch board</p><h2 className="mt-3 max-w-[10ch] text-5xl font-extrabold leading-[.9] tracking-[-.07em]">Find a seed worth answering.</h2><p className="mt-5 max-w-sm text-sm leading-relaxed text-[#d9d2cb]">Read the brief. Listen for the opening. Respond only when the work asks for your voice.</p><span className="mt-8 inline-flex items-center gap-2 rounded-full bg-[#e55b4c] px-5 py-3 text-sm font-bold">Browse seeds <ArrowRight className="h-4 w-4" /></span></Link><div className="grid gap-5"><Link href="/authors/collaborations/continuations" className="focus-house rounded-[1.5rem] border-2 border-[#d6cbb9] bg-[#fff4e6] p-7"><Inbox className="text-[#3e8074]" /><h2 className="mt-8 font-display text-3xl italic">Your review desk</h2><p className="mt-2 text-sm text-[#77717a]">See the continuations waiting on your eye.</p></Link><Link href="/authors/work" className="focus-house rounded-[1.5rem] border-2 border-[#d6cbb9] bg-[#f0c85c] p-7"><Users /><h2 className="mt-8 font-display text-3xl italic">Work in motion</h2><p className="mt-2 text-sm">Solo pieces and Tandem projects, together.</p></Link></div></div><div className="reveal reveal-2 mt-10 flex flex-wrap items-center gap-3 border-t-2 border-[#d6cbb9] pt-6 text-sm text-[#77717a]"><LockKeyhole className="h-4 w-4 text-[#e55b4c]" />Private by design · visible only to the people in the room · welcome, {user?.firstName || 'writer'}</div></Frame>;
}
function CreateSeed() {
...
function SeedDetail() {
  ...
  return <Frame eyebrow="Pitch board / read only" title={s.sourceProjectTitle} intro={`A seed by ${s.creatorName}. Read it as given; this is the shape the creator is protecting.`}>
  ...
  return <Frame eyebrow="Pitch board / response" title={locked ? "Your continuation is in review." : "Make an opening."} intro={locked ? "This submission is locked while the creator reviews it. The frozen seed remains read-only." : "This is a private draft. Nothing leaves your desk until you choose to submit it."}>
    <div className="reveal reveal-1 mt-10 grid gap-6 lg:grid-cols-[.7fr_1.3fr]">
      ...
      <div className="rounded-[1.5rem] border-2 border-[#d6cbb9] bg-[#fff4e6] p-7">
        ...
        {!locked && <div className="mt-6 flex flex-wrap gap-3"><button onClick={saveDraft} disabled={save.isPending || create.isPending || !text.trim()} className="focus-house rounded-full border-2 border-[#292b45] px-5 py-3 text-sm font-bold">{save.isPending ? 'Saving…' : 'Save draft'}</button><button onClick={submitDraft} disabled={!text.trim() || submit.isPending || create.isPending} className="focus-house rounded-full bg-[#e55b4c] px-5 py-3 text-sm font-bold text-[#fff4e6]">{submit.isPending ? 'Submitting…' : 'Submit continuation'}</button></div>}
      </div>
    </div>
  </Frame>;
}
