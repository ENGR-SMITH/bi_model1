...
// This is the read-only filmstrip: scrub across the finished cut to review the
// look, compare grades, and pick the cover frame. No in-browser grading —
// grades/captions/lower-thirds are authored in the desktop finish and land
// here via import as a new version.
// ---------------------------------------------------------------------------
...
function FinishPreview({
  ...
  return (
    <div className="paper-card" data-testid="panel-finish-preview">
      ...
      <p className="setting-copy">Scrub across the finished cut — each clip shows its own grade, with lower thirds and captions overlaid exactly as they render.</p>
...
// ---------------------------------------------------------------------------
// Thumbnail + exports — server-side job queue (no in-browser editing). The
// cover frame is picked by scrubbing the preview to the wanted frame; export
// fans the master out to every aspect ratio.
// ---------------------------------------------------------------------------
...
function ExportPanel({
  ...
  projectId: string;
  canEdit: boolean;
  /** Live playhead from the review preview — the frame the cover is cut from. */
  playheadMs: number;
  /** The asset under the playhead (source of the extracted cover frame). */
  ...
  // The cover is extracted from the frame under the review playhead.
  const runThumbnail = () => {
    if (!currentAssetId) return;
    thumbQueue.mutate(
      { projectId, data: { assetId: currentAssetId, timeMs: Math.round(playheadMs) } },
      {
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: getListVideoJobsQueryKey(projectId) });
        },
  ...
  const latestThumb = (jobs.data ?? []).filter((job) => job.type === 'THUMBNAIL').sort((a, b) => b.createdAt.localeCompare(a.createdAt))[0];
  ...
  return (
    <div className="paper-card accent-card">
      ...
      <div className="mt-3">
        <span className="mono-label">Cover frame</span>
        <div className="mt-2 flex flex-wrap items-center gap-3">
          <span className="den-tag teal" data-testid="thumbnail-frame">
            <ImageIcon size={11} />
            current frame @ {formatTimecode(playheadMs)}
          </span>
          {canEdit && (
            <button type="button" onClick={runThumbnail} disabled={!currentAssetId || thumbQueue.isPending} className="secondary-btn" data-testid="button-extract-thumbnail">
              <ImageIcon size={13} className={thumbQueue.isPending ? 'animate-pulse' : ''} />
              {thumbQueue.isPending ? 'Extracting…' : 'Extract thumbnail from this frame'}
            </button>
          )}
        </div>
        <p className="den-footnote mt-2">
          <ImageIcon size={12} />
          Scrub the preview to the frame you want, then extract it as the cover.
          {latestThumb && ` · latest extract: ${latestThumb.status.toLowerCase()}`}
        </p>
      </div>
...
export default function ContentCreatorsFinishPage() {
  ...
  // The FINISH leg's head snapshot — read-only. New versions arrive via import
  // (push a finished master) and the review/approve merge, never in-browser.
  const headSnapshot = useMemo<FinishSnapshot>(() => {
    const snapshot = finishTimeline.data?.snapshot as unknown as FinishSnapshot | undefined;
    return {
  ...
  // The asset under the playhead — the source of the extracted cover frame.
  const currentAssetId = useMemo(() => {
    const clips = [...headSnapshot.clips].sort((a, b) => a.inMs - b.inMs || a.id.localeCompare(b.id));
    const active = clips.find((c) => playheadMs >= c.inMs && playheadMs < Math.max(c.inMs + 1, c.outMs));
    return active?.assetId ?? clips[0]?.assetId ?? project.data?.assets[0]?.id ?? '';
  }, [headSnapshot.clips, playheadMs, project.data?.assets]);
  ...
  const oracleContext = useMemo(() => {
    const clips = headSnapshot.clips.map((c, i) => `clip ${i + 1}: ${assetsName(c.assetId)} · LUT ${c.grade.lut} · exposure ${c.grade.exposure} · warmth ${c.grade.warmth}`).join('\n') || 'none yet';
    const cards = headSnapshot.lowerThirds.map((l) => `“${l.title}${l.subtitle ? ` — ${l.subtitle}` : ''}” @ ${formatTimecode(l.startMs)}–${formatTimecode(l.endMs)}`).join('\n') || 'none yet';
    const caps = headSnapshot.captions.enabled ? headSnapshot.captions.style : 'off';
    return [
      ...
      `Lower thirds:\n${cards}`,
      `Captions: ${caps}`,
      `Thumbnail: ${headSnapshot.thumbnail ? `frame @ ${formatTimecode(headSnapshot.thumbnail.timeMs)}` : 'not marked'}`,
    ].join('\n\n').slice(0, 12000);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [headSnapshot, project.data?.name, project.data?.assets, timelineDuration]);
  ...
  const headVersionId = finishTimeline.data?.versions.find((v) => v.version === finishTimeline.data?.version)?.id ?? null;
    ...
    <div className="page">
      ...
      <div className="page-header">
        <div>
          ...
          <h1>Finish &amp; polish.</h1>
          <p>Scrub the graded preview, compare grades A/B across versions, extract the cover from any frame, and queue every export format. Grading happens in your desktop finish — push it here as a version.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <span className={`den-tag ${canEdit ? 'teal' : 'muted'}`}>
            <Check size={10} />
            {canEdit ? 'Motion & Color Director' : 'Viewing'}
          </span>
        </div>
      </div>
