...
function ProjectActivityPage() {
  ...
  return <Frame eyebrow="Nexet / activity" title="A clear record." intro="Who did what, when. Summaries only — hidden prose never enters this log."><ProjectTabs projectId={projectId} active="/activity" /><div className="reveal reveal-1 mt-8 max-w-3xl space-y-3">{q.isLoading ? <Loading /> : q.isError ? <ErrorState retry={q.refetch} /> : events.length ? events.map((event) => <div key={event.id} data-testid={`activity-${event.id}`} className="flex items-start gap-4 rounded-[1.25rem] card-surface p-5"><PiClockCountdownDuotone className="mt-0.5 h-4 w-4 shrink-0 text-[#3b82f6]" /><div><p className="text-sm font-bold">{event.summary}</p><span className="mt-1 block font-mono-ui text-[9px] uppercase tracking-[.12em] text-zinc-600">{event.eventType.replaceAll('_', ' ')} · {new Date(event.createdAt).toLocaleString()}</span></div></div>) : <Empty title="Nothing recorded yet." body="Contract approvals, submitted passes, and shared bible notes will appear here as the room moves." />}</div></Frame>;
}
